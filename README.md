...
Install the skill so Claude can invoke it on request (e.g. "transcribe this mp3"):

```bash
git clone https://github.com/JudaB/ai-transcribe-skill.git \
  ~/.claude/skills/transcribe-audio
```

Then ask Claude to transcribe any local audio file and it will run the script,
poll to completion, and report the output files.
